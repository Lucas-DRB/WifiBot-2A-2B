...
/*######################################################################################################################################*/
/*        Librairire de Navigation Autonome du WIFIBOT : Février 2017 :                                                                 */
/*																																	    */
/*######################################################################################################################################*/
...
// Fonction Avancer : faire avancer le robot 
void WifibotClient::Avancer(int left_speed, int right_speed)
{
	unsigned char flags = 128 + 64 + 32 + 16 + 1;
	SendCommand(left_speed, right_speed, flags);
}

// Fonction Reculer : faire reculer le robot
void WifibotClient::Reculer(int left_speed, int right_speed)
{
...
void WifibotClient::TournerDroite(int left_speed, int right_speed)
{
	unsigned char flags = 128 + 64 + 32 + 1;
	SendCommand(left_speed, right_speed, flags);
}

// fonction tourner à gauche
void WifibotClient::TournerGauche(int left_speed, int right_speed)
{
...
void WifibotClient::ArreterRobot()
{
	unsigned char flags = 1;
	SendCommand(0, 0, flags);
}

// fonction acceler le robot avec une acceleration donnée
void WifibotClient::AvancerAccelerer(int left_speed, int right_speed, int acceleration_speed)
{
	int g = left_speed + acceleration_speed;
	int d = right_speed + acceleration_speed;
	Avancer(g, d);
}

// fonction de deceleration du robot en faisant baisser sa vitesse
void WifibotClient::AvancerDecelerer(int left_speed, int right_speed, int deceleration_speed)
{
	if ((left_speed >= deceleration_speed) && (right_speed >= deceleration_speed))
		Avancer(left_speed - deceleration_speed, right_speed - deceleration_speed);
	else
		printf("The speed can't have a negative value");
}

/*__________________________________________________________________________________________________________________*/
/*                                                                                                                  */
/*                                              Navigation Approche Logique											*/
/*__________________________________________________________________________________________________________________*/
SensorData sensors_data;

// algo de navigation autonome basé sur une pparoche logique 
void WifibotClient::ApprocheLogiqueNavigation(int left_speed, int right_speed)
{
	GetSensorData(&sensors_data); //récupération des valeurs des 2 capteurs
...
/*__________________________________________________________________________________________________________________*/
/*                                                                                                                  */
/*                                             Filtrage des capteurs IR												*/
/*__________________________________________________________________________________________________________________*/
//Fonctions de filtrage des capteurs IR aifn d'éviter faux positifs/ faux négatifs
//Récupère les données des capteurs IR, et renvoie des données corrigées
#define IR_THRESHOLD 35
int iLastValue_Left = 0;
int iLastValue_Right = 0;


int WifibotClient::FiltrageCapteurIR_Right(int iRightIR) {
	//init for first itteration
	if (iLastValue_Right == 0)
	{
		iLastValue_Right = iRightIR;
	}
	//If we have a 10cm jump between two measurements : false measurement
	if ((iLastValue_Right + 10 < iRightIR)|| (iLastValue_Right - 10 > iRightIR))
	{
		return iLastValue_Right;
	}
	else {
		return iRightIR;
	}
...
int WifibotClient::FiltrageCapteurIR_Left(int iLeftIR) {
	//init for first itteration
	if (iLastValue_Left == 0)
	{
		iLastValue_Left = iLeftIR;
	}
	//If we have a 10cm jump between two measurements : false measurement
	if ((iLastValue_Left + 10 < iLeftIR) || (iLastValue_Left - 10 > iLeftIR))
	{
		return iLastValue_Left;
	}
	else {
		return iLeftIR;
	}
